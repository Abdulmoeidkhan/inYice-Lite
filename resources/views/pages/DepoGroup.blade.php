    ...
    <div class="card w-100">
        <div class="card-body p-4">
            ...
            <div class="row">
                ...
                ==="bxssUser"|| session('user')->roles[0]->name === "depo" || session('user')->roles[0]->name === "depoRep")
                <div class="d-flex flex-wrap">
                    @if($depoGuestRemaing>0)
                    <a type="button" href="{{route('pages.addDepoGuestRender',$id)}}" class="btn btn-primary mb-2">Add
                        Staff</a>&nbsp;
                    @endif
                    @if(session('user')->roles[0]->name === "admin" || session('user')->roles[0]->name === "depo" || session()->get('user')->roles[0]->name ==="bxssUser")
                    <button id="sent" class="print-action-button btn btn-primary mb-2">Print Bagde</button>&nbsp;
                    <button class="print-action-button-with-pic btn btn-success mb-2">Print Bagde With Picture</button>&nbsp;
                    @endif
                </div>
                @endif
            </div>
